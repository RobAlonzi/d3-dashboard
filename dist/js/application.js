...
var createPoints = function createPoints() {
    ...
    svg.append("text").datum([ptsBreakdown][0]).attr("transform", function (d) {
        return "translate(" + x(d[81][0]) + "," + y(d[81][1]) + ")";
    }).attr("dy", "0.35em").style("font", "10px sans-serif").text(function (d) {
        return 'Our Team (' + d[81][1] + ' pts)';
    });

    svg.append("text").datum([ptsBreakdown][0]).attr("transform", function (d) {
        return "translate(" + x(d[81][0]) + "," + y(d[81][2]) + ")";
    }).attr("dy", "0.35em").style("font", "10px sans-serif").text(function (d) {
        return 'Division Rival #1 (' + d[81][2] + ' pts)';
    });

    svg.append("text").datum([ptsBreakdown][0]).attr("transform", function (d) {
        return "translate(" + x(d[81][0]) + "," + y(d[81][3]) + ")";
    }).attr("dy", "0.35em").style("font", "10px sans-serif").text(function (d) {
        return 'Division Rival #2 (' + d[81][3] + ' pts)';
    });

    svg.append("g").attr("transform", "translate(0," + height + ")").call(d3.axisBottom(x));

    svg.append("text").attr("transform", "translate(" + width / 2 + " ," + (height + margin.top + 7) + ")").style("text-anchor", "middle").text("Games Played");
